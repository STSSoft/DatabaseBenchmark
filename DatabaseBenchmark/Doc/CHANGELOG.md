...
ver.3.0.1 (unreleased)

Task: да се уеднакви сериализацията на базите. Да се използва кодът от DatabaseBenchmark.Core.
Task: да се оправи Elapsed Time в долната лента. Понякога изчезва и при Resize на прозореца се появява.
Task: Връщане в Benchmark Layout. 
Task: Скриване на  TreeView buttons  в SingleTest Layout.
Task: Проблем с SingleTest button  в  PropertiesFrame.
Task: Edite PropertyGridUtils.

* ADDED: Two layout types  - Benchmark and  Single Test.
* ADDED: SingleTestFrame Form for single test layout.
* BUG FIXED: After load of New Project, TreeView is expanded.

* ADDED: WriteConcern property for MongoDB/TokuMX.
* ADDED: MongoDB/TokuMX now have a PerformReplace flag. If the flag is true (default), the database will perform replace operation
if it encounters a duplicate key; otherwise (false) it will discard the duplicate.

* BUG FIXED: JsonUtils -> Settings key in the JSON for the databases must be present even if the Settings property is null.
* BUG FIXED: ReportForm appears even if a test failed, which is not correct.
* BUG FIXED: When exporting, "summary" or "detailed" was not added to the filename.

* IMPROVED: Database versions are now included into the exported CSV results.
* IMPROVED: Layout and project serializations.

* UPDATED: Added link for the benchmark results in the ReportForm.
